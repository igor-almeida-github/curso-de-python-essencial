...
# Ao realizar o import de todo o módulo, das as funções, atributos, classes e propriedades que estiverem dentro do
# módulo ficarão disponíveis (Ficarão em Memória). Caso você saiba quais funções você precisa utilizar deste
# módulo, então esta não seria a forma ideal de utilização. Nós veremos uma forma melhor na Forma 2
# random() -> Gera um número real pseudo-aleatório entre 0 e 1.
print(random.random())
# OBS: Veja que para utilizar a função random() do pacote random, nós colocamos o nome do pacote
# e o nome da função, separados por ponto. Não confunda a função random() com o pacote random.
# Pode parecer confuso, mas a função random() é apenas uma função dentro do módulo random.
'''
